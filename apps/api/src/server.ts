...
async function fileText(file:Express.Multer.File){ if(file.buffer.subarray(0,4).equals(Buffer.from("%PDF"))) return (await pdf(file.buffer)).text; if(file.buffer[0]===0x50 && file.buffer[1]===0x4b) return (await mammoth.extractRawText({buffer:file.buffer})).value; if(isPlainText(file.buffer)) return file.buffer.toString("utf8"); throw new Error("Unsupported binary upload."); }
